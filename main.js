...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function callGemini(prompt) {
    if (!GEMINI_API_KEY) return "Erro: Chave da API não configurada.";

    const MODEL_NAME = "gemini-pro"; // ÚNICO modelo compatível
    ...
    try {
      ...
      // RETORNO DO JSON COMPLETO — SEM FILTRAR
  ...
  function aiMove() {
    ...
    if (d === "dificil" && ball.vx > 0) {
      ...
      if (d === "facil" && Math.random() < 0.004) {
        aiPaddle.y += (Math.random() * 220 - 110);
      }
    }
  ...
  document.querySelectorAll(".back").forEach(b =>
    b.addEventListener("click", () => { state.running = false; })
  );
